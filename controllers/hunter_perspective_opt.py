...
SCORE_RED = 1000 # score for encountering a red object
...
DATA_DIR = "/home/hjpae/hoss-iit/animats/log/genome_hunter_v3.1_opt.csv"
...
class Perspective:
    ...
    def __init__(self, risk_bias):
        ...
        """
        self.risk_bias = risk_bias  # how agent perceives red risk
    ...
    def expected_red_reward(self, score_red=SCORE_RED, penalty_red=PENALTY_RED, penalty_chance=PENALTY_RED_CHANCE):
        ...
        """
        base_ev = (1 - penalty_chance) * score_red + penalty_chance * (-penalty_red) # base expected value of red
        bias = (3 * self.risk_bias) * abs(base_ev)  # exaggerate or dampen the expected value of red
        # original multiplier was 2, but now 3 
        return base_ev + bias

    def adjust_fitness(self, red_score, green_score, penalty_chance, penalty_red, score_red, reward_green):
        # perspective-conditioned expected value
...
class GeneticAlgorithm:
    ...
    def evaluate_fitness(self, agent, generation):
        ...
        for i, weights in enumerate(self.population):
            ...
            steps, red_score, green_score, red_collision_penalty, revisit_penalty, idle_count = agent.run_simulation(nn, generation=generation, individual_idx=i)
            fitness = self.calculate_fitness(steps, red_score, green_score, revisit_penalty, idle_count, red_collision_penalty)
            fitness_scores.append(fitness)
        return np.array(fitness_scores)

    def calculate_fitness(self, steps, red_score, green_score, revisit_penalty, idle_count, red_collision_penalty):
        ...
        fitness = total_score - collision_penalty - revisit_penalty - idle_penalty
        return fitness
    ...
    def run_simulation(self, nn, generation=0, MAX_STEPS=MAX_STEPS, individual_idx=None):
        ...
        while self.supervisor.step(self.time_step) != -1:
            ...
            if steps % 200 == 0:
                ...
                print(f"Individual {individual_idx}, Gen {generation}, Step: {steps}, IdleSteps: {self.idle_count}, RScore: {self.red_score}, GScore: {self.green_score}, Penalty: {self.red_collision_penalty}, RPenalty: {self.revisit_penalty}")
            steps += 1
        ...
        print(f"Individual {individual_idx}, Gen {generation}, Final RScore: {self.red_score}, Final GScore: {self.green_score}, Final Penalty: {self.red_collision_penalty}, Final RPenalty: {self.revisit_penalty}, Final IdleSteps: {self.idle_count}")
        hidden_state_log = np.array(hidden_state_log)
        return steps, self.red_score, self.green_score, self.red_collision_penalty, self.revisit_penalty, self.idle_count
